...
import { getAdsbPosition, haversineKm, isTaxiingOrRolling, hasLiftedOff } from './adsbLol.js'
...
const TERMINAL_STATUSES = new Set([
  'landed', 'arrived', 'Arrived', 'Landed', 'cancelled', 'Cancelled',
])

// ── Budget guard ─────────────────────────────────────────────────────────────

async function canEnrich(state: PhaseState): Promise<boolean> {
  if (state.enrichmentCount >= MAX_ENRICHMENTS) {
...
async function pollAdsb(flight: { registration: string | null; ident: string }) {
  const reg = flight.registration ?? ''
  // Detect ICAO hex: exactly 6 hex characters
  const isIcaoHex = /^[0-9a-fA-F]{6}$/.test(reg)
  return getAdsbPosition({
    icaoHex: isIcaoHex ? reg : null,
    registration: !isIcaoHex && reg ? reg : null,
...
async function processFlight(flight: {
  ...
  if (!state) {
    ...
    console.log(`[orchestrator] Tracking flight ${flight.ident} — initial phase: ${computedPhase}`)
  }

  // Detect phase transitions (poller may have updated DB fields)
  if (computedPhase !== state.phase) {
  ...
  switch (state.phase) {
    ...
    case 'IN_TERMINAL': {
      ...
      if (isTaxiingOrRolling(pos) || hasLiftedOff(pos)) {
        ...
        if (await canEnrich(state)) {
          const enrichment = await fetchGateEnrichment(flight.faFlightId ?? flight.ident)
          if (enrichment) {
            await prisma.flight.update({
              ...
              data: {
                ...
                departureActual:   enrichment.estimatedOut      ?? undefined,
                status:            enrichment.status            ?? undefined,
              },
            })
    ...
    case 'EN_ROUTE': {
      const isShared = (await prisma.shareToken.count({ where: { flightId: flight.id } })) > 0
      ...
      if (destCoords) {
        ...
        if (deviation > ETA_DEVIATION_MS && await canEnrich(state)) {
          const enrichment = await fetchGateEnrichment(flight.faFlightId ?? flight.ident)
          if (enrichment) {
    ...
    case 'ARRIVING': {
      ...
      const hasLanded =
        pos.onGround === true ||
        (pos.altitudeFt !== null && pos.altitudeFt < 500)
        ...
        if (await canEnrich(state)) {
          ...
          await prisma.flight.update({
            ...
            data: {
              ...
              arrivalActual:   enrichment?.estimatedIn     ?? new Date(),
              status:          'landed',
            },
          })
          console.log(
          ...
          await prisma.flight.update({
            where: { id: flight.id },
            data: { landingActual: new Date(), status: 'landed' },
          })
